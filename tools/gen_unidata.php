 ...
 *
 * Output the various mapping classes used by the TLN_Normalizer class to the "Symfony/Resources/unidata" directory.
 *
...
foreach ( $lines as $line ) {
	...
	if ( isset( $matches[2] ) ) {
		...
		for ( $i = $begin; $i <= $end; $i++ ) {
			$exclusions[ tln_utf8_chr( $i ) ] = true;
		}
	} else {
		$exclusions[ tln_utf8_chr( hexdec( $matches[1] ) ) ] = true;
	}
}
...
foreach ( $lines as $line ) {
	...
	$code = tln_utf8_chr( hexdec( $parts[0] ) );
	...
	if ( $decomp ) {
		...
		$decomps = array_map( 'tln_utf8_chr', $decomps );

...
foreach ( $out_idxs as $idx ) {
	...
	foreach ( $tmp_codepoints as $codepoint ) {
		if ( $codepoint === $last + 1 ) {
			...
			if ( null === $carry ) {
				$ranges[] = tln_utf8_ints( $last );
			} else {
				if ( $first + 1 === $carry ) {
					$ranges[] = tln_utf8_ints( $first );
					$ranges[] = tln_utf8_ints( $carry );
				} else {
					tln_utf8_ranges( $ranges, $first, $carry );
				}
				$carry = null;
			}
			$first = $codepoint;
	...
	//error_log( "ranges=" . print_r( tln_array_map_recursive( 'tln_utf8_preg_fmt', $ranges ), true ) );

	// Generate the regular expression alternatives.

	$regex_alts[ $idx ] = tln_utf8_regex_alts( $ranges );
	...
	foreach ( $out_idxs as $idx ) {
		...
		foreach ( $tmp_codepoints as $codepoint ) {
			if ( $codepoint === $last + 1 ) {
				...
				if ( null === $carry ) {
					$regex_alts .= tln_unicode_preg_fmt( $last );
				} else {

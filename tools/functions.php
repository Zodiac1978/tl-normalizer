...
define( 'TLN_UTF8_MAX', 0x10ffff ); // Maximum legal unicode codepoint in UTF-8.
 ...
 */
function tln_utf8_ranges( &$ranges, $c1, $c2 ) {
	return tln_utf8_4range( $ranges, tln_utf8_ints( $c1 ), tln_utf8_ints( $c2 ) );
}

/**
 * Calculate ranges between UTF-8 ints arrays.
 */
function tln_utf8_4range( &$ranges, $ints1, $ints2, $i = 0 ) {
	...
	if ( $i === 3 ) {
		$ranges[] = tln_utf8_range( $ints1, $ints2, $i );
		return $ranges;
	}

	if ( $ints1[ $i ] === $ints2[ $i ] ) { // If main index the same, just do lower indexes.
		tln_utf8_4range( $ranges, $ints1, $ints2, $i + 1 );
		return $ranges;
	}

	if ( 0 === $ints1[ $i ] ) { // Need to span byte range.
		tln_utf8_4range( $ranges, $ints1, tln_utf8_replace( $ints1, $i + 1, $minmaxs[ $i + 1 ][1] ), $i + 1 ); // Range up to highest previous byte range.
		tln_utf8_4range( $ranges, $minmaxs[ $i ][0], $ints2 ); // Range from lowest current byte range.
		return $ranges;
	}
	...
	if ( 0x80 === $ints1[ $i + 1 ] ) { // Optimize to not output separate range if 1st int array at lowest value.
		...
		if ( array_slice( $minmaxs[ $i ][0], $i + 1 ) === array_slice( $ints1, $i + 1 ) && array_slice( $minmaxs[ $i ][1], $i + 1 ) === array_slice( $ints2, $i + 1 ) ) {
			$ranges[] = tln_utf8_replace( $ints1, $i, array( array( $ints1[ $i ], $ints2[ $i ] ) ), null, 0 ); // One part.
			return $ranges;
		}

		$ints1[ $i ] -= 1; // Fall through to processing of middle & last part with $ints1[ $i ] + 1.
	} else {
		tln_utf8_4range( $ranges, $ints1, tln_utf8_replace( $ints1, $i + 1, array(), null, 0, 0xbf ), $i + 1 );
	}

	// Middle and last part.
	if ( $ints1[ $i ] + 1 === $ints2[ $i ] ) { // Optimize to not output separate range if same.
		tln_utf8_4range( $ranges, tln_utf8_replace( $ints2, $i + 1, array(), null, 0, 0x80 ), $ints2, $i + 1 );

	} elseif ( $ints1[ $i ] + 1 === $ints2[ $i ] - 1 ) { // Optimize to not insert inner range if contiguous.
		$ranges[] = tln_utf8_replace( $ints1, $i, array( $ints1[ $i ] + 1 ), null, 0 );
		tln_utf8_4range( $ranges, tln_utf8_replace( $ints2, $i + 1, array(), null, 0, 0x80 ), $ints2, $i + 1 );
		...
		if ( array_slice( $minmaxs[ $i ][1], $i + 1 ) === array_slice( $ints2, $i + 1 ) ) { // Optimize to not output separate range if 2nd int array at highest value.
			$ranges[] = tln_utf8_replace( $ints1, $i, array( array( $ints1[ $i ] + 1, $ints2[ $i ] ) ), null, 0 );

		} else { // The full Monty.
			$ranges[] = tln_utf8_replace( $ints1, $i, array( array( $ints1[ $i ] + 1, $ints2[ $i ] - 1 ) ), null, 0 );
			tln_utf8_4range( $ranges, tln_utf8_replace( $ints2, $i + 1, array(), null, 0, 0x80 ), $ints2, $i + 1 );
		}
	}
...
/**
 * Helper for tln_utf8_4range() to insert values into ints array.
 */
function tln_utf8_replace( $ints1, $i1 = 0, $ints2 = array(), $len = null, $i2 = null, $fill = array( 0x80, 0xbf ) ) {
...
/**
 * Helper for tln_utf8_4range(). Given UTF-8 ints arrays, if there's a range between them at $i then insert (else just return first).
 */
function tln_utf8_range( $ints1, $ints2, $i = 0 ) {
 ...
 */
function tln_unicode_preg_fmt( $c ) {
 ...
 */
function tln_utf8_preg_fmt_range_entry( $range_entry ) {
	if ( is_int( $range_entry ) ) {
		return tln_utf8_preg_fmt( $range_entry );
	}

	if ( $range_entry[0] === $range_entry[1] ) {
		return tln_utf8_preg_fmt( $range_entry[0] );
	}

	if ( $range_entry[0] + 1 === $range_entry[1] ) {
		return tln_utf8_preg_fmt( $range_entry[0] ) . tln_utf8_preg_fmt( $range_entry[1] );
	}
 ...
 */
function tln_utf8_trie( &$trie, $range, $idx = 0, $parent = null, $fmt = null ) {
	$cnt = count( $range );
	...
	if ( $idx === $cnt - 1 && $parent ) {
		...
		$parent[ $fmt ] .= tln_utf8_preg_fmt_range_entry( $range[ $idx ] );

	} elseif ( $idx < $cnt && is_array( $trie ) ) {
		// Create key entry (trie).
		$fmt = tln_utf8_preg_fmt_range_entry( $range[ $idx ] );
		if ( ! isset( $trie[ $fmt ] ) ) {
			$trie[ $fmt ] = array();
		}
		tln_utf8_trie( $trie[ $fmt ], $range, $idx + 1, $trie, $fmt );
	}
}
 ...
 */
function tln_utf8_trie_regex_alts( $trie ) {
	$ret = array();
	...
	foreach ( $trie as $key => $val ) {
		...
		if ( is_array( $val ) ) { // Recurse.
			if ( count( $val ) > 1 ) {
				$ret[] = $key . '(?:' . implode( '|', tln_utf8_trie_regex_alts( $val ) ) . ')';
 			} else {
				$ret[] = $key . implode( '', tln_utf8_trie_regex_alts( $val ) );
			}
 ...
 */
function tln_utf8_regex_alts( $ranges ) {
 ...
 */
function tln_utf8_rand_ratio_str( $utf8_len = 10000, $other_ratio = 0.001, $other_strs = null, $other_is_codepoints = true ) {
	...
	if ( $other_is_codepoints ) {
		for ( $i = 0; $i < $other_len; $i++ ) {
			$ret[] = tln_utf8_chr( $other_strs[ rand( 0, $other_max ) ] );
		}
	} else {
		for ( $i = 0; $i < $other_len; $i++ ) {
